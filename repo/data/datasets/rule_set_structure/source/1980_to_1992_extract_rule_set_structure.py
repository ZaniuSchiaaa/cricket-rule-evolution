...
def extract_law_num(text):
    match = re.search(r'\bLAW (\d+)\b', text)
    if match:
...
# I think there are no bullet points! Hooray!
...
with open(f"{tree_folder}/{cleaned_file_name}.txt", 'r') as f:
    ...
    enum = 'LAW 1'
    enum_type = 'law' # won't matter
    depth_dict = {'law': 1, 'num': 2, 'letter': 3, 'roman': 4} # for reg, not for notes
    ...
    for line in f: 
        ...
        if line.startswith("LAW"): 
            ...
            was_notes = is_notes
            para_counter = 0
            yaml_file_contents = yaml_file_contents + f'  - LAW {law_num}:\n' # make node
            # enum = f"LAW {law_num}"
            # enum_type = 'law'
        ...
        if line.startswith("NOTES"): is_notes = True
