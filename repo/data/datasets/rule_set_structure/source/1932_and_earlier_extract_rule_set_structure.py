...
desired_year_label = "1902"
worded_titles = ["ONE DAY MATCHES.", "SINGLE WICKET.", "THE BATSMAN.",
                 "THE BOWLER.", "THE BOUNDARY.", "UMPIRES.", "INSTRUCTIONS TO UMPIRES."]
...
def write_in_new_paras(curr_depth, num_paras, existing_str, is_title, is_notes): 
    ...
    for i in range(num_paras): 
        ...
        if is_notes: 
            new_para_entry = new_para_entry + f'- NP{i+1}\n'
        else: 
    ...
    if (num_paras == 0 and not is_title): 
...
with open(f"{tree_folder}/{cleaned_file_name}.txt", 'r') as f:
    ...
    # Note that there are rogue lines on the side, so this might be tricky. 
    # If start with Note or Notes (with a space after), then it doesn't count as para. 
    # I think would be nice to still indicate which ones are regular paras and
    # which ones are note paras...

    # In the meantime, the #.— pattern persists. 
    # Ok, the rogue lines don't change paragraphing thankfully. They also appear
    # pretty inconsistently...which doesn't really matter here I guess.
    ...
    for line in f: 
        ...
        if match:
            # Write in new paras
            yaml_file_contents = write_in_new_paras(1, para_counter, yaml_file_contents,
                                                    is_title, is_notes)
            is_notes = False
            is_title = False
            para_counter = 0
            yaml_file_contents = new_little_law(yaml_file_contents, match.group(1), 1)


        caps_match = caps_pattern.search(line)
        # The following three clauses go together! Take note. 
        if line.startswith("Notes") or line.startswith("Note"): 
            # Add paras from the previous part that may/may not have had notes
            yaml_file_contents = write_in_new_paras(1, para_counter, yaml_file_contents,
                                                    is_title, is_notes)
            # Now, status update of is_notes. 
            is_notes = True
            para_counter = 0 # reset paras so the numbering restarts. 
            # DON'T increment para counter. This skips the else clause. 

        elif line.strip() in worded_titles: 
            yaml_file_contents = write_in_new_paras(1, para_counter, yaml_file_contents,
                                                    is_title, is_notes)
            yaml_file_contents = new_little_law(yaml_file_contents, line.strip(), 1)
            is_notes = False
            is_title = True
            para_counter = 0
            # DON'T increment para counter. This skips the else clause. 
            ...
            pass # do nothing; crucially don't increment the para_counter

        else: 
            if ends_with_period(line): para_counter += 1

    yaml_file_contents = write_in_new_paras(1, para_counter, yaml_file_contents, is_title, is_notes)
