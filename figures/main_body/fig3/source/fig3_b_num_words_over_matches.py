...
Computes and plots the number of words for each cricket ruleset edition year,
based on the plain-text ruleset files, against cumulative matches played.

Usage
-----
    # From the repo root:
    python figures/main_body/fig3/source/fig3_b_num_words_over_matches.py 

Outputs
-------
    figures/main_body/fig3/components/fig3_b_num_words_over_matches.svg
"""
...
def build_dataframe() -> pd.DataFrame:
    ...
    for year in TARGET_YEARS:
        record = {"Year": year, "Number of Words": np.nan}
...
def plot(df: pd.DataFrame) -> None:
    ...
    ax.set_ylabel(r"\# of words", fontsize=30, labelpad=10)
    ax.grid(False)
    ...
    fig.savefig(os.path.join(FIGURES_DIR, "fig3_b_num_words_over_matches.svg"))
    plt.close(fig)
