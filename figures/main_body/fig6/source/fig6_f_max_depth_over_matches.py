...
Plot the maximum depth of each cricket ruleset tree against the cumulative
number of matches played.

Usage
-----
    # From the repo root:
    python figures/main_body/fig6/source/fig6_f_max_depth_over_matches.py

Outputs
-------
    ./figures/main_body/fig6/components/fig6_f_max_depth_over_matches.svg
"""
...
VIS_FILEPATH = "./figures/main_body/fig6/components/fig6_f_max_depth_over_matches.svg"

INDENT_UNIT = 2  # spaces per nesting level in the flattened YAML files

COLUMN = "Maximum Depth"
COLOR = "#7F7F7F"
...
def compute_maximum_depth(yaml_text):
    """
    Compute the maximum node depth of a YAML tree from its raw text.

    Each non-blank, non-comment line is treated as one node; its depth is
    inferred from leading indentation. Returns the deepest depth across all
    nodes, or 0 if the tree is empty.
    """
    max_depth = 0
    found_node = False
    ...
    for line in yaml_text.splitlines():
        ...
        if stripped == "" or stripped == "---" or stripped.startswith("#"):
            continue
        found_node = True
        max_depth = max(max_depth, line_depth(line))

    return max_depth if found_node else 0
...
def build_depth_dataframe(yaml_dir):
    """Build a DataFrame of maximum depth per year from flattened YAML trees."""
    ...
    for file in os.listdir(yaml_dir):
        ...
        df.loc[year, COLUMN] = compute_maximum_depth(yaml_text)
...
def merge_with_matches(df, matches_csv):
    """Join the maximum-depth data with cumulative-matches data, indexed by year."""
...
def plot_depth(merged_df, vis_filepath):
    """Scatter the maximum depth against cumulative matches played (log x-axis)."""
    ...
    ax.set_ylabel("Maximum depth", fontsize=30, labelpad=10)
    ax.grid(False)
