...
Compute and plot the number of communities over time, based on the greedy-modularity community partition.

Usage
-----
    # From the repo root:
    python figures/main_body/fig9/source/fig9_f_num_communities_over_time.py

Outputs
-------
    ./figures/main_body/fig9/components/fig9_f_num_communities_over_time.svg
"""
...
NUM_COMM_SVG = os.path.join(OUTPUT_DIR, "fig9_f_num_communities_over_time.svg")
...
def num_communities(G, weight):
    G_undirected = G.to_undirected()
    communities = nx.community.greedy_modularity_communities(G_undirected, weight=weight)
    return len(communities)


def compute_num_communities_series(graphs, weight):
    """Return a number-of-communities dict keyed by year."""
    num_communities_series = {}
    for year in sorted(graphs):
        n = num_communities(graphs[year], weight)
        num_communities_series[year] = n
        print(f"{year}: {n} communities")
    return num_communities_series
...
def main():
    ...
    num_comm_label = r"\# of communities" if USE_TEX else "# of communities"

    num_communities_series = compute_num_communities_series(graphs, WEIGHT_KEY)
    plot_series(num_communities_series, num_comm_label, NUM_COMM_SVG)
