...
Plots the average weight of existing edges in the interdependency network over
time (one point per edition year). Edge weights are read directly from the
multi-count GEXF graph files.

Statistic
---------
    Average weight (existing edges) = total_weight / num_edges,
    i.e. the mean weight over only those dyads that have a non-zero-weight edge.

Usage
-----
    # From the repo root:
    python figures/supplemental/figS1/source/figS1_c_ave_weight_over_existing_edges_over_time.py

Outputs
-------
    ./figures/supplemental/figS1/components/figS1_c_ave_weight_over_existing_edges_over_time.svg
"""
...
def avg_weight_existing(G):
    """(3) Mean weight over existing edges only."""
...
if not gexf_files:
    ...
    for fpath in gexf_files:
        ...
        G = load_graph(fpath)
        values.append(avg_weight_existing(G))
        years.append(year)
    ...
    ax.set_ylabel("Average weight (existing edges)", fontsize=30, labelpad=10)
    ax.tick_params(axis="x", labelsize=18)
    ax.tick_params(axis="y", labelsize=18)

    out_path = os.path.join(OUTPUT_DIR, "figS1_c_ave_weight_over_existing_edges_over_time.svg")
