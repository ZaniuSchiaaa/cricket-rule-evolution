...
Plots the number of edges in the interdependency network over time (one point per
edition year). Edges are read directly from the multi-count GEXF graph files.

Statistic
---------
    Number of edges = count of existing (non-zero-weight) directed edges.

Usage
-----
    # From the repo root:
    python figures/supplemental/figS1/source/figS1_b_num_edges_over_time.py

Outputs
-------
    ./figures/supplemental/figS1/components/figS1_b_num_edges_over_time.svg
"""
...
if not gexf_files:
    ...
    ax.set_ylabel(r"\# edges", fontsize=30, labelpad=10)
    ax.tick_params(axis="x", labelsize=18)
    ax.tick_params(axis="y", labelsize=18)

    out_path = os.path.join(OUTPUT_DIR, "figS1_b_num_edges_over_time.svg")
